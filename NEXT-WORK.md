...
> - ⏳ 待交易日（周一盘中）复验：实算数值跨日回归、强度差分真实行情表现、事件捕获口径。
...
| 维度 | 状态 | 证据 |
| --- | --- | --- |
| 形态 | DSH Web 插件 · 主布局第四列 stock（layout-patch 引擎自愈，版本门控 0.1.1-rc.2） | README §布局；lib/index.js 已构建 |
...
| C5 | 遗留文件未清理：旧 `Watchlist.tsx`(M3 走后端)、`src/test-*`、`mock-*`、`tmp-old-*.tsx`、旧 `src/index.css`、`dist/` | git untracked / 目录清单 | 提交基线前删/移，测试脚手另归档 |
