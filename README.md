...
- **三栏联动 + 键盘优先**：`selection.ts` 的「当前标的」是唯一真源（点行 → 主图/下部面板/右栏 AI 一起换）；
  ...
  底栏：阶段 / 当前标的、`持久化：host|本地`、`v1.4.0 · <build id>`（有新构建时提示刷新）、🩺 诊断面板。
...
当前版本 **1.4.0**（`package.json` 为唯一来源）= v1.2 官方槽迁移 + v1.3 宽视图沉浸式/AI 双通道 + v1.4 流程驱动导航
的**合并发布**。文档只写批次/版本名，不重复绝对数字：版本史见 `CHANGELOG.md`，下一批见 `docs/ROADMAP.md`。
