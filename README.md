...
一级导航只回答「**现在该干什么**」，功能页降级为查证工具：

```
┌ 状态带：阶段(可点，看本时段该答什么)│[复盘][作战][看盘]│[工具]│指数条│广度│情绪≈│[问模型][⌘K] ┐
├──────────┬──────────────────────────────────────────────┬──────────────────────┤
│ 左栏      │ 阶段头：本阶段必须回答的问题 + 唯一输出          │ 右栏 AI 研判          │
│ 自选/涨停 │ 主区 ── 复盘：七步流程（列流并排，⑦输出独占整行） │  一键研判 → 决策卡     │
│ /异动     │        作战：竞价→验证窗→盘中→尾盘（时段自切）   │  关键价位 → 画到主图   │
│ 实时价    │        看盘：左列表 + 主图 + 资金/逐笔/竞价      │  深入对话 → 注入对话   │
└──────────┴──────────────────────────────────────────────┴──────────────────────┘
```
...
| **看盘** | 任意时段 | 自由查看：左栏点票 → 主图 + 资金/逐笔/竞价 + 右栏 AI（不属于流程） |
| **工具** | 任意时段 | 总览 / 指数 / 涨停梯队 / 外盘 / 选股筛选 / 自选盘 / 监控规则 / 个股明细 |

- **时段自动跟随 + 手动优先**：跨过 9:15 / 14:30 / 15:10 边界自动切入口；用户手动点过导航后本会话停止跟随。
- **三栏联动**：`src/lib/selection.ts` 的「当前标的」是唯一真源——左栏点一行 → 主图/下部面板/右栏 AI 一起换，**不切页**。
- **键盘优先**：`⌘K`/`/` 搜索（可搜功能名直达工具）、`1-3` 入口、`t` 工具、`j/k` 移动、`a` 问模型、`[`/`]` 收栏、`Esc` 关抽屉。
- **底栏**：阶段 / 工具 / 当前标的、`持久化：host|本地`、`v1.4.0 · <build id>`（服务端已重建时提示「有新构建 · 点此刷新」）、以及 **🩺 诊断面板**（构建一致性 / 持久化 / 数据链路 / 缓存底账 / AI 与 HIST）。

### 目标形态（v1.5，见 `docs/ROADMAP.md` A4）

| 变更 | 内容 |
| --- | --- |
| 左栏 | **自选栏保留**（点行即渲染个股信息）；**新增「个股」分组** = 最近看过的个股（本地缓存，点行同样渲染）；**去掉「涨停」「异动」分组**（涨停已有梯队页；异动并入工具单页） |
| 工具 | 8 个工具页**合并为一张单页**（分区展示，⌘K 直达区块）；**外盘弱化**为页内次要区块 |
| 主区 | 选中标的即在主区渲染**个股信息**（现「看盘›工作台 / 明细」的分工在新版收口） |
...
CI（`.github/workflows/ci.yml`）：install(→prepare build) → `tsc --noEmit` → 体积护栏 → 两个离线冒烟。
**当前无 ESLint、无测试框架**——纯函数单测与 lint 是 `docs/ROADMAP.md` B4 的内容。
