...
- **行情页 = 三块一页**（市场总览 / 指数 / 涨停梯队，合并决策）。合并**不等于**把三个轮询拼一屏，
  页内定了三条纪律：① **滚到可视区才挂载**（`IntersectionObserver`，240px 预热）；
  ② 离开视野**停轮询**但保留数据（不因上下滚动反复重取）；③ **全页只有一个节拍器**
  （15/20/30/60s 或暂停）——否则同一份指数数据会被两个订阅者各拉一遍。页头把
  「当前活跃区块数 / 节拍 / 上次刷新时间」摊开显示。三个旧工具 id（总览/指数/涨停梯队）
