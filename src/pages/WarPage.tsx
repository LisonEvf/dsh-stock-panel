...
import { callToolJson, type AShareRow } from '@/lib/stock-data'
...
import { fetchAllA, computeBreadth, type Breadth } from '@/lib/market'
import { getLatestPlan, getReview, subscribeReview, today } from '@/lib/review-store'
import { getDayRun, subscribeDayRun, clearAiAnswers } from '@/lib/dayrun'
...
import { WarPlanCard } from '@/components/WarPlanCard'
import { useAiTask } from '@/lib/ai-task'
import { warPlanOf } from '@/lib/ai'
...
import { collectWarMaterials } from '@/lib/war-plan-collect'
import { tradingWindow } from '@/lib/mission'
...
export function WarPage({ onOpenStock }: Props) {
  ...
  /* ── v1.6 作战思路（模型给的"今天怎么打"）状态 ──────────────────────────────
   *
   * 分工：本页只负责"什么时候生成、素材从哪来、结果怎么落到当日记录"；
   * 组装/存档/采纳在 `lib/war-plan.ts`，素材采集在 `lib/war-plan-collect.ts`，
   * 呈现与护栏展示在 `components/WarPlanCard.tsx`。
   */
  const aiPlan = useAiTask('war-plan')
  const [planErr, setPlanErr] = useState<string | undefined>(undefined)
  const [planMissing, setPlanMissing] = useState<string[]>([])
  /** 符号 → 名称（模型只给符号，名称一律从素材回查；查不到就显示符号本身）。 */
  const [planNames, setPlanNames] = useState<Map<string, string>>(() => new Map())
  const [autoWhy, setAutoWhy] = useState('')
  const [manualOpen, setManualOpen] = useState(false)
  /**
   * "正在生成"（**从采集素材那一刻起**，不是从模型调用起）。
   *
   * 真机实测（acceptance/probe-war-plan.mjs）：采集要拉自挖类引擎（冷启动可达数十秒），
   * 而 `useAiTask.busy` 只在模型调用期间为真 —— 于是采集期卡片看起来是"还没生成"，
   * 用户再点一次「生成」什么也不会发生（被防重入挡住了），表现得像点坏了。
   * 所以这里自己持有一个覆盖全流程的忙标志。
   */
   ...
   * 本轮数据快照（供生成思路时组装素材）。
   *
   * 为什么用 ref 而不是 state：`load` 每 30s 重写它，若进 state 会让 `generate`
   * 的身份每轮都变 → 自动生成那个 effect 反复重跑（且每次都"看起来没跑过"）。
   */
  ...
  }>({ breadth: null, regime: null, situation: null, ladder: null, events: [], pulses: [], allRows: null })
  /** 全 A 快照到位序号（0 = 还没取到）：自动生成要靠它判断"素材在手"。 */
  const [dataSeq, setDataSeq] = useState(0)
  /** 本会话已为本时段自动生成过（避免轮询把 effect 反复触发成多次模型调用）。 */
  const autoKeyRef = useRef('')
  /** 正在生成（防重入：模型调用是本页最贵的一次动作）。 */
  const generatingRef = useRef(false)
  /** 生成时的取消句柄（改素材/卸载时中止采集）。 */
  const planAbortRef = useRef<AbortController | null>(null)
  /** `aiPlan.run` 的稳定引用（见下方 effect；避免把 useAiTask 的对象塞进依赖数组）。 */
  const aiRunRef = useRef(aiPlan.run)
  useEffect(() => {
    aiRunRef.current = aiPlan.run
  }, [aiPlan.run])
  ...
  // 作战思路存档订阅（跨组件/跨视图同步；也覆盖 host 域 hydrate 后的权威版本）
  useEffect(() => subscribeWarPlan(force), [])
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      ...
      // v1.6：把本轮素材交给 ref（作战思路的组装要用它；不进 state 以免 generate 身份乱跳）
      dataRef.current = {
        breadth,
        regime: r,
        situation: s,
  ...
  /**
   * 生成（或重算）一次作战思路 —— **本页唯一会调用模型的地方**。
   *
   * 一次生成 = 采集素材（自挖类 + 竞价，≤9 次工具调用，见 `lib/war-plan-collect.ts`）
   * + 一次模型调用；成功后按"只填空位"写进当日记录（`adoptWarPlan`），
   * 并把整份思路连素材缺口一起存档（按时段缓存，重复打开 = 0 次调用）。
   *
   * 依赖刻意为空（除 `day`）：时段/素材/持仓都从 ref 与 store 现读 —— 否则
   * 每 30s 的轮询都会换掉这个函数的身份，自动生成那条 effect 会跟着重跑。
   */
  const generate = useCallback(
    async (opts: { force?: boolean } = {}) => {
      if (generatingRef.current) return
      const nowMs = Date.now()
      const st = stageOf(phaseRef.current, minuteOfDay(new Date(nowMs)))
      const cur = getWarPlan(day, st)
      // 同一时段已有新鲜思路：不重复烧调用（手动重算传 force 才越过）
      if (opts.force !== true && cur !== null && planFreshness(cur, nowMs) === 'fresh') return
      ...
      setPlanErr(undefined)
      try {
        const d = dataRef.current
        const minutes = minuteOfDay(new Date(nowMs))
        const win = tradingWindow(minutes)
        const materials = await collectWarMaterials({
          day,
          stage: st,
          phase: phaseRef.current,
          clockText: hhmm(new Date(nowMs)),
          windowNote:
            phaseRef.current === 'auction'
              ...
              : phaseRef.current === 'trading'
                ? win === 'preOpenWindow'
                  ? '开盘验证窗 9:30–10:00'
                  : win === 'tail'
                    ? '尾盘动作点 14:30–15:00（T+1 下唯一的动作点）'
                    : '盘中主段（只答三问）'
                : `${STAGES[st].label}（${STAGES[st].window}）`,
          breadth: d.breadth,
          regime: d.regime,
        ...
        setPlanMissing(materials.missing)
        setPlanNames(nameIndexOf(materials))

        const res = await aiRunRef.current(ctx)
        if (res === null || ac.signal.aborted) return // 被取消（切时段/卸载/重复点）
        if (!res.ok) {
          setPlanErr(res.error ?? '模型生成失败')
          return
        }
        const parsed = warPlanOf(res)
        /**
         * 结构化失败也要留痕：把"模型回包不可用"记成一条 `insufficient` 的思路，
         * **原文照样入库**（`raw`），卡片会把原文显示出来 —— 绝不让模型说的话凭空消失。
         */
        const plan: WarPlan = parsed ?? unusableWarPlan('模型回包不是可用的作战思路 JSON（原文保留在下方，可点重算）')
        // 采纳：只填当日记录里的空位（人工答案一律保留，见 adoptWarPlan 的注释）
        const adopted = adoptWarPlan(day, plan, getPositions())
        saveWarPlan({
          day,
          stage: st,
          at: nowMs,
          ...
          ms: res.meta?.ms ?? 0,
          // 上下文体积由 host 回传（前端不再为此把 ai-contract 打进 client bundle）
          ctxBytes: res.meta?.contextBytes ?? 0,
          shrunk: res.meta?.shrunk ?? [],
          // 白名单规模由 host 半的护栏报告回传（前端不再自己重算一遍素材池）
          pool: { symbols: plan.guard.symbolPool, sectors: plan.guard.sectorPool },
          missing: materials.missing,
          stageLabel: STAGES[st].label,
          adopted,
        })
        setAutoWhy(`本时段已于 ${hhmm(new Date(nowMs))} 生成（${st === 'auction' ? '竞价' : STAGES[st].label}）`)
        force()
      } catch (e) {
        // 采集阶段的失败（引擎/行情/竞价）—— 也走错误态 + 重试，不静默
        if (!ac.signal.aborted) setPlanErr((e as Error)?.message ?? '作战思路生成失败')
      } finally {
  ...
  /* ── v1.6 作战思路：本时段的存档 + 自动生成纪律 ─────────────────────────────
   *
   * "页面打开 / 时段切换各自动跑一次"由 `autoGenerateDecision` 判定（纯函数，单测在
   * tests/war-plan.test.ts）：同段已有缓存 → 不跑；没有盘面素材（休市/取数失败）→ 不跑
   * （那只会换来一句"证据不足"，白花一次调用）；其余 → 跑。
   * `autoKeyRef` 是第二道闸：effect 会随轮询重跑（dataSeq 变），但同一时段只自动生成一次。
   */
  const planRec = getWarPlan(day, stage)
  /**
   * 素材是否在手（决定"要不要自动生成"）。
   *
   * 读 `dataRef` 而不是 state：本页没有把广度/温度计放进 state（它们只在 load 里用），
   * 而 `dataSeq` 就是"ref 刚被写过"的信号 —— 它进 effect 依赖，等价于"数据到位了"。
   */
  const hasMaterials = dataSeq > 0 && (dataRef.current.regime !== null || dataRef.current.breadth !== null)
  useEffect(() => {
    if (!phaseKnown) return
    const decision = autoGenerateDecision({
      hasMaterials,
      existing: getWarPlan(day, stage),
      now: Date.now(),
    })
    setAutoWhy(decision.why)
    if (!decision.run) return
    const key = planKey(day, stage)
    if (autoKeyRef.current === key) return
    autoKeyRef.current = key
    void generate()
  }, [day, stage, phaseKnown, hasMaterials, dataSeq, generate])
  ...
  return (
    <div className="h-full overflow-y-auto px-2.5 pb-3">
      ...
      {/* ── v1.6 作战思路（本页主产物）────────────────────────────────────────
          以前这里是"数据摊开 + 你自己点选"；现在第一屏是模型读完其他板块给的思路，
          用户只做采纳 / 修改 / 驳回。放在最上面是刻意的：它是"今天怎么打"的答案，
          其余卡片是为核对这个答案服务的（温度计/事件流/持仓决策台仍在下方）。
          整行铺开（不进 `.dc-flow`）：它是本页唯一的"结论级"内容，与并排看的数据不同级。 */}
      <WarPlanCard
        record={planRec}
        busy={generating || aiPlan.busy}
        error={planErr ?? (aiPlan.status === 'error' ? aiPlan.error : undefined)}
        availability={aiPlan.availability}
        autoWhy={autoWhy}
        adoption={planRec?.adopted ?? null}
        missing={planMissing}
        nameOf={(s) => planNames.get(s)}
        inWatchlist={(s) => watchlist.some((w) => `${w.market}${w.code}` === s)}
        onRegenerate={() => void generate({ force: true })}
        onReject={() => {
          clearAiAnswers(day)
          force()
        }}
      ...
      {/* N8 昨日预期 × 今日竞价判定（竞价/盘前展示，逐条写回 dayrun）
          v1.6：其中"模型已按竞价特征给过初判"的那几条会带「模型」徽标（见组件内）。 */}
      ...
      {/* N8 盘中三问必答卡：盘中只答 Q1-Q3（系统局势候选仅在温度计有数据时提供）
          v1.6：模型已按默认写进当日记录（标 from:'ai'），这张卡**降级为人工覆盖入口** ——
          默认收起，点开即改；「不是用户自己选择」说的是"不必自己选"，不是"不许自己改"。 */}
      {phase === 'trading' && (
        <>
          <button
            type="button"
            className="mb-1 flex w-full items-center gap-1 rounded border border-slate-100 bg-slate-50/60 px-2 py-1 text-left dc-t-data font-medium text-slate-400 hover:bg-slate-100"
            onClick={() => setManualOpen((v) => !v)}
            title="模型给的三问落点已写进当日记录；这里可以逐条改成你自己的判断"
          >
            <span>{manualOpen ? '▾' : '▸'}</span>
            人工覆盖三问 / 持仓动作
            <span className="dc-t-micro text-slate-300">
              （模型结论在上方「落点」；点任意选项即覆盖，改完仍标为你自己的判断）
            </span>
          </button>
          {manualOpen && <QAnswers autoSituation={regime ? situation : null} />}
        </>
      )}
