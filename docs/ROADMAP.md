...
| --- | --- |
| `guard.ts` | 引文逐字反查（分隔符归一）、可计算证据分 **0.40 覆盖度 + 0.25 命中票数 + 0.20 窗口内比例 + 0.15 条数（3 条封顶）**、时间窗护栏、降级成因分层、**自报 confidence 不作门控**、**主题名归一**（脱引号/去空白/去「板块」这类通用后缀；过长只提示不截断） |
| `prompt.ts` / `parse.ts` / `fingerprint.ts` / `materials.ts` | 提示词与语料渲染；分级降级解析；口径指纹（provider/模型/窗口/源集合/**提示词版本**/护栏阈值）；板块类型码、**非叙事板块词黑名单**、去重/配额/截断、**`belong_board` 契约漂移检测**（字段名/类型码取错要报"本地漂移"而不是"没有板块"） |
| `structure.ts`（✅ 2026-09-14 新增） | **结构标签**：成员所属官方行业/概念板块里被 **≥2 只成员**共享的词 → 「这个班像什么」。**确定性、不经模型、不依赖新闻、任何 `as_of` 都成立**，从批量命名已采到的语料里算（0 额外请求）。与模型主题名并列回传、各自标来源 —— 它回答的是"像什么"，不是"为什么一起动" |
| `collect.ts` / `run.ts` / `route.ts` | 采集（见下方「时间性」）；取类成员 → **伪类拒绝**（`weak_chain` + `lib/concept-quality.ts` 的传递链判据）→ 采集 → 模型 → 护栏 → **进程内缓存**（key = asOf:classId:fingerprint，命中不产生第二次 LLM 调用）；✅ 批量路径 `nameClasses`（类列表默认）：一次并集采集 → 按成员数分批问模型 → **逐组独立过护栏**（跨组借证据会被拒；漏项如实降级）+ **逐组结构标签** + 整批 memo（重复打开 0 调用）。GET 出能力与口径（模型/提示词版本/默认参数/批量预算/护栏/缓存规模）；POST 命名（单类 `classId`，或批量 `all:true` / `classIds:[..]`；非法一律 400，不猜默认值） |
| UI 与强度口径 | 类列表 `ConceptClassesCard` 作为「自挖板块」一级入口（✅ 打开即批量命名归类、按**强度**降序、行内给涨幅/涨停数/成员，含被排除的**弱链/传递链**类计数与池内孤立票；✅ 2026-09-14 版面重做：三列骨架 + 强度条 + 名称来源胶囊 + 成员截断展开，颜色全走 `--dc-*` token 跟随主题）；⚠️ 个股卡 `StockConceptCard`（**设计上的主视角**：所属类 + 同类伙伴 + 最近共动邻居 + 命名）**当前没有挂载点** —— 它只被 `StockDetailPage` 引用，而 A6 摘掉了那个整页的入口（文件留着但 tree-shaking 后**不进 bundle**）。结论展示抽成 `NamingResultPanel` 两处共用。强度 = `lib/concept-strength.ts`（纯函数，host/client 共用）：**均涨幅% + 6×涨停数**，涨停按 `buy_price_limit` 精确判定、缺失值不按 0 算；`hist_concept_classes` 出数时由 `host/hist-data.ts` 的 `enrichClasses` 补齐（与引擎共用 60s 全A快照 memo）。单测：`tests/concept-strength.test.ts` / `tests/concept-quality.test.ts` |
...
**仍未做**：① 命名缓存只在进程内存（跨进程持久化要新增表 + 版本迁移，等真实使用反馈）；② 官方关系三分类（收敛/叙事漂移/官方盲区）—— v0 明确不做；③ 自挖类**跨日跟踪**（今天的"铜"与昨天的"铜"是不是同一个班 —— 命名缓存按 as_of 隔离，强度也只看当日；跨日归并需要额外的类相似度口径，未列入本批）；④ **结构标签暂时只在命名请求里回传**（它与批量命名共用那一次采集，0 额外请求）；模型不可用/命名桥接未注册时这一页仍只有成员名可看 —— 要让它独立于命名链路，得给 `hist_concept_classes` 加一轮 `belong_board` 采样（每类最多 N 只）+ 进程内缓存，属于请求预算的取舍，等真实使用反馈再定；⑤ **（2026-09-14 真机复核实测的新增项）把个股卡挂回「工作台」** —— 它是设计上的**主视角**（"这只票今天跟谁一起动"），但 A6 摘掉「个股明细」整页后失去挂载点：真机取证确认工作台 DOM 里没有它（二级面板只有 资金流/逐笔/竞价，`同类伙伴`/`所属共动类` 等字符串在 `lib/client.js` 里搜不到），而**数据源是通的**（`hist_concept_query` 真机可用、冷启动 ~6s）。挂回 `WatchView` 必须连 `NamingResultPanel` 一起进 client bundle，而体积只剩 ~13KB 余量（651.7 / 665KB）→ **先减重、再挂载**，否则就得抬护栏（抬护栏要按 `check-bundle-size.mjs` 的规矩在文件头留账）。**注意**：这也是本节「验收锚点」① 之外的诚实性锚点 —— 文档写着"已上线"但实机跑不到，正是这一条要防的。

**验收锚点**：① 命名结果带可回溯引文，编造引文的负样本必须降级为 `no_common`；② 时间窗外的证据不计入门控（负样本可复现）；③ 与官方花名册的「盲区」判定只对**行业口径**生效；④ 缓存命中不产生重复 LLM 调用（指纹不匹配视为未命中）；⑤ UI 上明确区分「自挖类（市场今天认定的班）」与「官方概念」，且**不自动改写**用户的复盘结论；⑥ **（2026-09-14 追加）任何 `as_of` 下每行都必须有可读标签** —— 模型主题名拿不到时由结构标签顶位，且来源必须标出来（模型主题 / 官方行业 / 官方概念）；⑦ **（2026-09-14 追加）假数据必须照真字段名写**，`belong_board` 的字段名/类型码漂移要报"本地漂移"而不是"没有板块"（那次事故让整张列表一个名字都没有）。覆盖：`tests/naming-guard.test.ts`（29 条）+ `tests/naming-pipeline.test.ts`（26 条，注入假工具/假模型跑全链路）+ `tests/naming-materials.test.ts`（10 条，字段名契约 + 结构标签）+ `tests/naming-cause.test.ts`（7 条）+ `tests/naming-news.test.ts`（21 条）+ `scripts/smoke-naming.mjs`（对**构建产物**验路由/口径/护栏/缓存/结构标签，CI 已接）。　**规模**：A2a ✅ / A2b ✅
...
| 行情合并 | 市场总览 + 指数 + 涨停梯队 → **一屏并排聚合**（`MarketPage`）：容器宽度分列 / 单节拍器 / 每块可折叠 | ✅ **2026-09-14 重排**：改为「环境带 + 指数│梯队 + 榜单│异动」（见 A7）—— 旧三块并排在 780px 真机下藏了 74% / 83% 的内容 |
...
- `MarketPage.tsx`：三块**并排**聚合（不是竖着摞），列数按**容器实测宽度**（`ResizeObserver`：≥1120 三列 / ≥760 两列 / 否则单列 —— 视口断点在 DSH 里会骗人：左栏 + 右 AI 栏都开着时视口 2560px、面板只有 600px）；请求预算纪律见 `ARCHITECTURE.md` §4 与 A7；
...
### A7 · 两个重点页的专项（自挖板块命名 + 行情重排）✅ **已完成（2026-09-14）**

**A7a 自挖板块：分类命名**（触发：真机 9 个可采信类全部显示「素材不足」，一个名字都给不出来）
- **真 bug**：`host/naming/materials.ts` 的 `boardNameOf` 读 `board_name ?? name`，真字段是 **`board_symbol_name`** → 板块归属素材恒为 0 条。它不是"少一类素材"：提示词第 0 条明写「没有快讯时用板块标签兜底」，而板块标签是**唯一任何 `as_of` 都采得到的源**，它一空，非交易日的类必然 `insufficient`。真机取证（`acceptance/probe-structure-live.mjs`）：修前 0 条 / 修后 **used 30 条（15/15 只票）**。
- **为什么单测没抓住**：假数据照着错代码写（`{ board_name }`），两边一起错就永远自洽。已把 `tests/naming-pipeline.test.ts` 与 `scripts/smoke-naming.mjs` 的假数据改成真字段名，并新增 `tests/naming-materials.test.ts` 反向钉住契约 + `boardContractDrift()`（字段名/类型码取错要报"本地契约漂移"，不许说成"没有板块"）。
- **结构标签**（`host/naming/structure.ts`）：成员所属**官方行业/概念板块**里被 **≥2 只成员共享**的词 → 「这个班像什么」。不经模型、不依赖新闻、任何 `as_of` 都成立，从批量命名已采到的语料里算（0 额外请求、0 额外调用）。真机 8/8 组拿到可读标签（`机床制造·通用设备` / `生物制品` / `5G概念·数据中心` / `半导体·半导体设备` …）。与模型主题名**并列**展示、各自标来源。
- **传递链伪类过滤**（`lib/concept-quality.ts`，host/client 同判据）：引擎的 `weak_chain` 漏掉一类 —— 实测 52 只 / 类内相关 0.428 / 强边密度 0.14 的巨类 `weak_chain=false`，照样进"可采信"并送去命名。判据 = `强边密度 < 0.5` **且** `类内相关 < 实际切边阈值`（单看一条会误杀"3 只票的三角形"）。界面计数 + 逐条原因，命名路径拒绝并给 `reason:"pseudo_class"`。
- **主题名归一 + 提示词形状约束**：脱引号/去空白/去「板块」后缀；提示词加第 7/8 条「2~8 字题材短名」，提示词版本 `a2b-1 → a2b-2`（不改版本号会让旧长句名字被当成新口径的缓存命中）。
- **版面重做**：一行三列骨架 + 相对强度条 + 名称来源胶囊 + 成员截断展开；颜色从 tailwind 固定色阶迁到 `--dc-*`（新增 `--dc-surface` / `--dc-surface-2`：宿主**亮色**下 layer-1/2/3 全是 `#ffffff`，直接拿它们当卡片底等于没有层级）。

**A7b 行情页：重新组合**（真机 780px 实测：旧三块并排藏了 74% / 83% 的内容）
- **新组合**：环境带（全宽 6 格 KPI + 涨跌分布横条，两个 2×2 的并集）→ 主行「指数 │ 涨停梯队」→ 底行「榜单 2×2 │ 市场异动」。实测藏留 0% / 19% / 0% / 0%；「涨停」在页面的出现次数 19 → 5。
- **梯队改 chip 分组**：69 只涨停股原来排 2174px，改成"一梯队一行 chips"+默认只展开最高两档（有标注的取舍，全量一次点击可达）。
- **顺带修真问题**：页头写着「全页共用一个节拍器」，但三块自带的 20/15/30s 定时器从没关掉（审计项 I-b，实际 4 个定时器在跑）→ 现在传 `pollMs={0}` 兑现，并用 `scripts/ux-contract.mjs` 的 `market-single-metronome` 钉住。
- **删掉的文件**：`src/pages/MarketOverview.tsx`（它的内容全部被环境带 / 榜单 / 异动接管，只被 `MarketPage` 引用）。
- **验收**：`acceptance/probe-market-page.mjs`（隔离 headless Chrome：块矩形 + 内容/可视高 + 明细计数 + 三条点击链路 + 明暗计算样式）、`acceptance/probe-concept-page.mjs`、`scripts/verify-live.mjs`、`acceptance/readme-shots.mjs`（README 截图重拍）。

　**规模**：M

### A8 · 作战板块重做（作战思路由模型给结论）✅ **已完成（2026-09-14）**

- **触发**：用户「重做作战板块，是大模型提供作战思路而不是用户自己选择，注意以其他板块为基础，而不是空穴来风」。
  （契约在实现前问过用户：产物 = 完整作战计划；人工输入保留为**可选覆盖**；生成时机 = 页面打开 + 时段切换各一次。）
- **改了什么**：旧版作战页只**展示**数据，决策劳动全在用户身上（复盘页手写预期清单、Q1/Q2/Q3 三张卡逐项点选、竞价逐条判定）；AI 直调通道一直开着，但作战页**一次模型都没调**。新版第一屏是模型读完**其他板块素材**给的作战思路（主攻方向 → 候选票含触发/失败条件 → 三问落点 → 持仓动作 → 竞价判定），用户只做采纳 / 修改 / 驳回。
- **四层分工**：素材（`lib/war-plan-collect.ts`：行情 / 自挖板块 / 选股筛选 / 复盘 / 竞价 / 持仓 / 自选）→ 组装（`buildWarContext`，显式声明 `sectorUniverse`）→ 护栏（`lib/war-plan-guard.ts`，**host 半权威执行**：票白名单 + 方向名逐字反查 + 引文反查，越界剔除并回传）→ 呈现与采纳（`components/WarPlanCard.tsx` + `adoptWarPlan`：只填空位、标 `from:'ai'`、人工答案绝不覆盖）。
- **成本纪律**：按 `${交易日}:${时段}` 缓存（同时段重复打开 = 0 次模型调用）；休市 / 无素材**不自动生成**；采集 = 1 次 `hist_concept_classes` + 竞价时段 ≤8 次 quote/auction，**不进 30s 轮询**。
- **真机取证**：`acceptance/probe-war-materials-live.mjs`（真数据源 + 假模型）实测采集 14.0s → 板块榜 12 / 梯队 20 / 自挖类 8（19 成员位，排除 2 个弱链类）/ 选股候选 8 / 上下文 7.7KB，护栏在真素材下正确剔除越界点名（白名单票 55 / 方向 12）；`acceptance/probe-war-plan.mjs`（隔离 headless Chrome）抓到并修掉"采集期假空闲"与"升级态错误出路"两个真问题。
- **遗留**：新建 host 表 `war_plans`（按日按时段存档）；体积累计 651.7KB（护栏 650 → 665KB，账写在 `scripts/check-bundle-size.mjs`，**下一批必须减重** —— 这也是把个股卡挂回「工作台」的前置条件，见 A2「仍未做」⑤）。　**规模**：L

### A5 · 样式 token 迁移收尾（`--dc-*` 逐页替换）

**现状**：只有 7 个新文件（`panel/*`、`views/*`）消费 `--dc-*`；**28 个旧文件仍有 1,000+ 处硬编码 Tailwind 颜色**（`ReviewPage` 单文件 136 处），CSS 里 64 条暗色主题重映射作为过渡层。
（2026-09-14 已顺手减 3 行：`bg-blue-100` / `bg-slate-50/80` / `divide-slate-100` 在源码里已零引用 —— 行情页迁到 token 后自然消失，按本节"每迁一页删对应行"的纪律删掉。）

**内容**：逐页迁移到语义 token，每迁完一页删掉对应重映射行；顺带做旧页面的宽屏精修（当前宽屏重排只落在复盘/作战/工作台主区）。
...
> 📌 **规划号 ≠ 发布号（2026-09-14 对账）**：上表里的 `V1.5 / V1.6 …` 是**批次代号**，不是发布版本号。
> 实际发布：**1.4.0**（2026-09-12，合并规划中的 v1.2/v1.3/v1.4）· **1.5.0**（2026-09-13，本表 V1.5–V1.8
> 的条目已大部分落地：B1/B2/B4/B5/B6 ✅、A1/A2/A4/A6 ✅、B3 部分 ✅，另含版面与契约收口）·
> **1.6.0**（2026-09-14，**A7 两个重点页专项 + A8 作战板块重做** —— 这两项都不在上表里，是用户直接点名的新需求，
> 见 §A7 / §A8）。
> 定版流程见 `CHANGELOG.md` 头部；**未落地项以各节标 ⏳ 的为准**，不以上表为准。
...
> 📌 **体积（现行值，2026-09-14 / 1.6.0）**：client.js **651.7 KB / 护栏 665 KB**（≈98%，余量 ~13KB）。
> 历史值 814.0KB / 840KB 是**减重前**的口径、534.3KB / 600KB 是 1.4.0 时的口径、600.7KB / 650KB 是 1.5.0 时的口径 ——
> 仅作对比，**勿再引用**（现口径以 `node scripts/check-bundle-size.mjs` 输出为准）。
> ⚠️ 余量只够小改动：**下一批先减重**（`pnpm bundle:report` 看成分），要挂回个股卡之类的额外代码必须先过这笔账。
...
| # | 事项 | 需要谁定 | 影响 |
| --- | --- | --- | --- |
| 1 | 自挖概念的**呈现密度**：是「当日类列表」为主，还是「个股所属共动类」为主？（A2b 可先做后者，成本低） | 用户 | 已定：**两者都要**（个股卡=主视角，类列表=一级入口且按强度排序）。⚠️ 2026-09-14 真机复核：**个股卡当前无挂载点**（见 A2「仍未做」⑤） |
