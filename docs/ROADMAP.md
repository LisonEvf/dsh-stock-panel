...
- `ToolHost.tsx`：⚠️ **只挂载当前展开的区块** —— 这不是偷懒而是硬约束：
  ...
  8 个一起挂会把请求预算打爆（`docs/ARCHITECTURE.md` §3.1）。因此：**标题常驻（信息架构可见）、
  内容按需挂载（离开即卸载 → 轮询停止）**；
...
✅ j/k 在左栏两组内移动；⏳ ⌘K 输入功能名直达区块（当前仍走工具单页的整体切换，未做区块级滚动定位）。

**规模**：M（已完成主体 S，剩余为主区收口与区块级直达）
