...
2. **A2b 命名与呈现 🟡 内核已落地（2026-09-12），接线待做**：host 半移植 `cluster-namer` 口径——
   ...
   缓存指纹。
   **已落地（`src/host/naming/`，纯函数 + 25 条单测 `tests/naming-guard.test.ts`）**：
   - `guard.ts`：引文逐字反查（分隔符归一，防"合规引文被误判幻觉"）、A5 可计算证据分
     （0.40 覆盖度 + 0.25 命中票数 + 0.20 窗口内比例 + 0.15 条数，条数 3 条封顶）、
     A6 时间窗护栏（窗口外证据不计入门控）、A7 降级成因分层、**自报 confidence 不作门控**；
   - `prompt.ts`：系统提示词（硬性规则与护栏一一对应）+ 语料渲染（素材体 `标题 / 摘要`）；
   - `parse.ts`：分级降级解析（围栏/前缀/尾逗号/括号栈补齐），失败如实降级并保留原文；
   - `fingerprint.ts`：口径指纹（provider/模型/窗口/源集合/**提示词版本**/护栏阈值）；
   - `materials.ts`：板块类型码（12 行业 / 4 概念 / 5 风格 / 3 地区）、**非叙事板块词黑名单**
     （真实踩过「含可转债」被当成主题）、去重（二元组 Dice ≈ difflib 0.9）/ 配额 / 截断。
   **本批修掉的两个移植偏差**（单测发现）：
   ① 参考实现按语料顺序取引文出处，而语料按时间升序 → 同一文本同时存在于窗口内外时，
   会先撞上最旧那条、把**本该算窗口内**的证据挡掉（冤枉降级）；已改为**优先认窗口内那条**；
   ② 参考实现的括号补齐是"先补 `}` 再补 `]`"，`{"a":[1,2`（截断在数组里）补出来仍非法；
   已改为按**括号栈**补（含未闭合字符串）。
   **校准给出的形态约束**（写进实现要求）：
   - 主视角 = **个股视角**（`hist_concept_query`：所属类 + top-k 共动邻居 + corr）——
     因为校准后仍有 132/191 只孤立，"类列表"信息量薄；
   - 类列表次要（4–6 个非弱链类），**必须标出 as_of 与四个参数**（同票不同参数所属类不同，不标=不可复现）；
   - 冷启动文案：首次 ~8s（拉数百只 K 线），之后 30–50ms（用 `hist_concept_status` 显示 ready/as_of）；
   - v0 **不做**与官方行业的重合度对照（那是命名阶段 `differ.py` 的活）。
   **接线待做**：① 素材采集（`unusual` 是**市场级实时列表**，无历史 → 历史 as_of 回放时只能靠
   日K推的封板状态，须显式降级为 `partial_material`）；② `POST /api/stock-panel/naming` 路由 +
   host 侧缓存（按 `asOf/classId` + 指纹）；③ 个股视角 UI + 类列表（标 as_of 与四参数）。
...
✅ A2b-①：命名内核（护栏 / 提示词 / 解析 / 指纹 / 素材归一）落地 + 25 条离线单测；
🟡 A2b-②：素材采集 + 路由 + 缓存 + 个股视角 UI（待做，见上）。

**规模**：A2a ✅ / A2b M（内核 ✅，接线剩余 M）
