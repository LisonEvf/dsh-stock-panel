...
> 📌 **规划号 ≠ 发布号（2026-09-13 对账）**：上表里的 `V1.5 / V1.6 …` 是**批次代号**，不是发布版本号。
...
> 📌 **体积（现行值，2026-09-13 / 1.5.0）**：client.js **600.7 KB / 护栏 650 KB**（≈92%）。历史值 814.0KB / 840KB 是**减重前**的口径、534.3KB / 600KB 是 1.4.0 时的口径，仅作对比，勿再引用（现口径以 `node scripts/check-bundle-size.mjs` 输出为准）。
