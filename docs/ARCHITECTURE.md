...
- **行情页是一页三块（A4 合并，用户决策）**：`MarketPage` = 市场总览 + 指数 + 涨停梯队。
  合并的理由是这三块回答同一条问题链（今天什么状态 → 指数怎么走 → 钱在哪些板上），
  拆成三个入口时用户必须在页签间来回跳。三个旧 id（`overview`/`indices`/`ladder`）由
  `toolIdOf()`/`LEGACY_TOOL_IDS` **就地映射到 `market`**（否则老用户落盘的旧值会被静默丢弃 → "工具页打不开"）。
...
- **请求预算纪律**：工具单页之所以"只挂载当前区块"，是因为 7 个页面的轮询叠加会打爆预算
  ...
  行情页合并时为此定了三条纪律（写在 `MarketPage` 头注释）：
  ① **滚动到可视区才挂载**（`IntersectionObserver`，`rootMargin 240px` 预热）；
  ② 离开视野**停止轮询**（`enabled=false`）但保留数据与 DOM —— 不因来回滚动反复重取；
  ③ **全页只有一个节拍器**（15/20/30/60s 或暂停，由 `tick` 驱动各区块强制验证）：
     否则同一份指数数据会被两个订阅者各拉一遍（`useSwr` 的 in-flight 去重只在同时发起时生效）。
  涨停梯队即使被更快节拍驱动也走自己的 30s 共享缓存 → 实际仍是 ~30s 一轮；页面顶部把
  「当前活跃区块数 / 节拍 / 上次刷新时间」摊开显示，让请求代价可见。
