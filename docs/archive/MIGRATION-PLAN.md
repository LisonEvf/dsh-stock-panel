> ## ⚠️ 已归档（2026-09-12）——仅作历史溯源，不要当作现状
>
> 本文是 **0.4.0 时代**的「从 SPA 迁移到 DSH 插件」实施计划，已被以下事实取代：
> 数据链路改为**进程内 embedded node-tdx**（§3 的远端 MCP 工具矩阵已失效）；
> 导航从「10 Tab 壳」改为**流程三入口 + 工具单页**（§4 的 PanelApp Tab 壳已不存在）；
> 体积阈值（400/450/600KB）三套数字均与实际护栏（820KB）不符。
> 现行文档：`README.md`（总览）· `docs/ROADMAP.md`（待做）· `docs/ARCHITECTURE.md`（架构）。
> 本文仍可查：M0–M11 批次定义、B 轨处置清单（§6）、代码规范（§7.2）、插件装配速查（§11）。
...
| --- | --- | --- |
| 形态 | 独立 React SPA + FastAPI + Polars/vectorbt | DSH Web GUI「A股工作台」视图标签页插件（官方 `conversation.view`；v1.1 及以前为补丁造的 `stock` 第四列） |
...
- **首屏**：视图标签页 300ms 内可见骨架，数据 1s 内出首屏（MCP 局域网 RTT ~ms 级）。
...
| 里程碑 | 内容 | 状态 |
| --- | --- | --- |
| M0 | 插件骨架 + layout patch（stock 第四列）+ host 半自愈 —— **v1.2 已废弃**：改官方 `conversation.view` 视图标签页，补丁/自愈/备份全删 | ✅（历史形态见 README §注入方案） |
...
```
DSH Web GUI 会话页视图标签页「A股工作台」（官方 conversation.view）
...
### M8 — 轻量选股引擎（P1，规模 L） ✅ 已交付（0.8.0）
- **实现**：lib/screener.ts（快照条件/6 预设/MA 金叉·破 MA60 信号）+ pages/ScoutPage.tsx（选股 Tab）。B 轨完整策略引擎仍待后端。
- **内容**：新增「选股」Tab：
...
### M10 — 扩展市场 + 系统打磨（P2，规模 S-M） ✅ 外盘部分已交付（0.9.0）
- **实现**：GlobalPage（美股/港股 报价+日K，期货异动快览）+ stock-data goods 适配器。竞价速览/角标打磨未做。
...
- **内容**：
  ...
  4. 注入契约自检：视图注册走官方 `conversation.view`（声明感知 `slots.inject`），
     运行时用 `window.__STOCK_PANEL__.viewRegistered` 判断是否注册成功；宿主升级无需跟进锚点
     （v1.2 起已无布局补丁）；
...
| 3 | DSH 升级改动 `conversation.view` 契约（如槽改名/移除） | 视图标签页不出现 | 声明感知 `slots.inject`：等不到声明就静默不注册，GUI 不受影响；`window.__STOCK_PANEL__.viewRegistered` 自检；契约变更时改一个常量（`VIEW_SLOT`/`VIEW_ID`）即可对齐 |
...
`/api/stock-panel/mcp` 桥接路由 404；面板从未出现（当时形态是 `stock` 第四列）。
...
- ~~ui-layout 布局补丁（stock 列）已打在 bundle 文件上，与插件加载无关。~~
  **v1.2 起不再有任何宿主 bundle 补丁**：界面注入改官方 `conversation.view` 视图标签页，
  宿主 `dsh-client-ui-layout/lib/client.js` 已还原为 pristine，`$DSH_HOME/patches` 已清空。
  视图注册与否只看 `window.__STOCK_PANEL__.viewRegistered`，与 bundle 字节无关。
