...
- **A4 行情页合并（市场总览 + 指数 + 涨停梯队 一页三块，用户决策）**：三块回答的是同一条问题链
  （今天什么状态 → 指数怎么走 → 钱在哪些板上），拆成三个入口时用户只能在页签间来回跳。
  **合并 ≠ 把三个轮询拼一屏**（涨停梯队单轮 ≤177 次工具调用），因此页内定了三条纪律并写进注释：
  ① **滚到可视区才挂载**（`IntersectionObserver`，`rootMargin 240px` 预热）；
  ② 离开视野**停轮询但保留数据与 DOM**（`enabled=false`）—— 不因上下滚动反复重取；
  ③ **全页只有一个节拍器**（15/20/30/60s 或暂停，`tick` 驱动各区块强制验证）—— 否则同一份指数
  数据会被两个订阅者各拉一遍（`useSwr` 的 in-flight 去重只在**同时**发起时生效）。
  涨停梯队即使被更快节拍驱动也走自己的 30s 共享缓存，实际仍 ~30s 一轮；页头把
  「活跃区块数 / 节拍 / 上次自动刷新时间」摊开显示，让请求代价可见。
  三个旧工具 id（`overview`/`indices`/`ladder`）由 `toolIdOf()` + `LEGACY_TOOL_IDS`**就地映射到
  `market`**：不映射的话老用户落盘的旧值会被 `setTool` 校验静默丢弃，表现为"工具页打不开"。
  状态带与总览的指数芯片改指向「行情」页。测试：`tests/selection.test.ts`（3 条：旧 id 映射 /
  合并后旧区块下线 / 非法 id 拒绝）。体积：client.js 558.0 → **567.1 KB**（护栏 600 KB，余量 33 KB）。
- **A4 行情面板改为「一屏并排聚合」**（更正上一批的竖排做法）：第一版把三块**竖着摞**成长页，
  用户指出要的是**在一个面板里聚合成宽屏仪表盘** —— 三块各占一列、各自内部滚动、**整页不滚动**，
  横向一眼扫完。落地：
  · 12 栅格并排（总览 4 / 指数 5 / 梯队 3 —— 指数最宽，它是唯一含图的块）；
  · **列数按容器实测宽度**（`ResizeObserver`）而不是 Tailwind 视口断点：DSH 左栏 + 右 AI 栏都在时
    视口 2560px 而面板只有约 600px —— 第一版因此出现"视口够宽 → 强制三列 → 每列挤成一团"
    （真机截图确认）；现在 ≥1120 三列 / ≥760 两列 / 否则单列（单列改回整页滚动 + 每块最小高度）；
  · 每块自带**本块刷新时间**（子页面 `onUpdatedAt` 回传）+ **单块刷新** + **折叠**（折叠即卸载、
    该块轮询停止）；聚合面板里总览不再重复显示 9 大指数条（旁边的「指数」块已含切换条 + 图）；
  · 请求预算：默认节拍取 **30s**（三块同时在跑是聚合的代价），页头显示「轮询中 N/3 · 节拍 · 上次刷新」。
  真机验证（浏览器实拍）：三块并排渲染、块时间戳分别 12:24:09 / 12:24:03 / 12:24:06、轮询 3/3、
  总览块已无重复指数条。体积：**568.8 KB**（护栏 600 KB，余量 31 KB）。
